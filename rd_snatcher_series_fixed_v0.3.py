...
def login_qbittorrent():
    r = session.post(f"{QBIT_HOST}/api/v2/auth/login", data={'username': QBIT_USER, 'password': QBIT_PASS})
    if 'Ok.' not in r.text:
...
class DownloadTask:
    ...
    def download(self):
        try:
            ...
            if r.status_code != 200:
                console.log(f'❌ Failed to unrestrict {self.filename}: {r.text}')
                self.status = "error"
                return

            unrestricted_data = r.json()
            ...
            # Diretório correto
            season, episode = extract_episode_info(correct_filename)
            series_name = extract_series_name(correct_filename)

            folder_path = os.path.join(SAVE_PATH, series_name)
            if season:
                folder_path = os.path.join(folder_path, f"Season {season}")
    ...
    def pause(self): self._pause.clear(); self.status = "paused"
    def resume(self): self._pause.set(); self.status = "downloading"
    def stop(self):
        self._stop.set()
...
def make_table():
    ...
    for i, task in enumerate(downloads):
        size = os.path.getsize(task.save_path) if task.save_path and os.path.exists(task.save_path) else 0
...
def user_controls():
    while True:
        command = input("\n(pause/resume/cancel) [id]: ").strip().split()
        if len(command) != 2:
            continue
        action, idx = command
        try:
