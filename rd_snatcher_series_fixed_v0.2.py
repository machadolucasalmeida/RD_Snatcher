...
def extract_series_and_episode_info(filename):
    # Ex: Stranger.Things.S01E02...
    match = re.search(r'(.+?)\.S(\d{2})E(\d{2})', filename, re.IGNORECASE)
    if match:
        series_raw = match.group(1)
        series_name = series_raw.replace('.', ' ').replace('-', ' ').title()
        season = int(match.group(2))
        episode = int(match.group(3))
        return series_name.strip(), season, episode
...
class DownloadTask:
    ...
    def download(self):
        try:
            ...
            # Detect info
            series_name, season, _ = extract_series_and_episode_info(correct_filename)
